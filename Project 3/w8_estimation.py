...
def variance(
    ...
    # Try to compute Hessian inverse if needed, fallback to pseudo-inverse on failure
    H_inv = None
    if cov_type in ['Hessian', 'Sandwich']: 
        ...
        try:
            ...
            print("Warning: Hessian is singular; using pseudo-inverse instead.")
            H_inv = la.pinv(H)
    ...
    if cov_type == 'Hessian':
        ...
        # Use pseudo-inverse as a robust fallback for singular B
        try:
            B_inv = la.inv(B)
        except la.LinAlgError:
            print("Warning: Outer product matrix is singular; using pseudo-inverse instead.")
            B_inv = la.pinv(B)
        cov = 1/N * B_inv
    elif cov_type == 'Sandwich':
        A_inv = H_inv #result.hess_inv
        # If H_inv came from pseudo-inverse, we still construct the sandwich estimator
