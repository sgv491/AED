...
def load_firm_data():
    ...
    # Independent variables: constant, capital, labor
    x = np.column_stack([
        np.ones(dat.shape[0]),         # constant
        dat["lcap"].values,            # log capital
        dat["lemp"].values             # log employment
    ])

    # Panel structure
    N = dat["firmid"].nunique()        # 441 firms
    T = dat["year"].nunique()          # 12 years
